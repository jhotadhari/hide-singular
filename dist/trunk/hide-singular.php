...
Plugin URI: https://waterproof-webdesign.info/wppr_project/hide-singular
Description: Hide singular view for a specific postHide singular view for a specific post
Version: 0.0.1
Author: jhotadhari
...
class Hisi_Hide_Singular {

	protected static $instance = null;
	const VERSION = '0.0.1';
	const DB_VERSION = 0;			// int	increase the number if the database needs an update
	const PLUGIN_SLUG = 'hide-singular';
	const PLUGIN_NAME = 'Hide Singular';
	const PLUGIN_PREFIX = 'hisi';
	...
	public function on_activate() {
		if ( $this->check_dependencies() ){
			...
			do_action('hisi_on_activate_before_flush');
			flush_rewrite_rules();
			$this->maybe_update();
			do_action('hisi_plugin_activated');
		} else {
			add_action( 'admin_init', array( $this, 'deactivate' ) );
			wp_die(
	...
	public function on_deactivate() {
		$this->add_roles_and_capabilities();
		do_action('hisi_on_deactivate_before_flush');
		flush_rewrite_rules();
		do_action('hisi_plugin_deactivated');
	}

	public static function on_uninstall() {
		do_action('hisi_plugin_uninstalled');
	}
	...
	protected function init_options() {
		update_option( 'hisi_version', self::VERSION );
		add_option( 'hisi_db_version', self::DB_VERSION );
	}
	...
	protected function maybe_update() {
		if ( get_option( 'hisi_db_version' ) < self::DB_VERSION ) {
			// require_once( self::plugin_dir_path() . 'inc/dep/class-hisi-update.php' );
			// new Hisi_Update();
			// class Hisi_Update()  is missing ??? !!!
		}
	}
...
function hisi_hide_singular_init() {
	return Hisi_Hide_Singular::get_instance();
}
hisi_hide_singular_init();
